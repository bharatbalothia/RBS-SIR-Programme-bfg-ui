...
public class TrustedCertificateDetails {
    ...
    private void validateCertificateUsingAugmentedService(CertificateValidationService certificateValidationService, X509Certificate x509Certificate,
        ...
        String errorMessage = (String) Optional.ofNullable(certificateChain.get("error"))
                .orElseGet(() -> Optional.ofNullable(certificateChain.get("message")).orElse(null));
        if (errorMessage != null)
            errors.add(Collections.singletonMap("authChainReport", Collections.singletonList(errorMessage)));
        else if (Boolean.valueOf(Optional.ofNullable(certificateChain.get("self-signed")).orElse("").toString()))
