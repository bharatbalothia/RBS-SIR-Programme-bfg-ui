...
class ImportedTrustedCertificateServiceTest {
    ...
    @Test
    void importCertificatesFromB2B_ShouldPersistTrustedCertificate() throws JsonProcessingException, InvalidNameException, NoSuchAlgorithmException, CertificateEncodingException {

        certificateDetails.setValid(true);
        when(certificateIntegrationService.getCertificates()).thenReturn(Collections.singletonList(importedCert));
        doNothing().when(trustedCertificateDetailsService).checkCertNameUniquenessLocally(
                any(TrustedCertificateDetails.class), anyString());
        when(trustedCertificateDetailsService.getTrustedCertificateDetails(any(X509Certificate.class), anyBoolean()))
                .thenReturn(certificateDetails);
        when(trustedCertificateService.save(any(TrustedCertificate.class))).thenReturn(trustedCertificate);
    ...
    @Test
    void importCertificatesFromB2B_ShouldIgnoreTrustedCertificate() throws JsonProcessingException, InvalidNameException, NoSuchAlgorithmException, CertificateEncodingException {

        certificateDetails.setValid(false);
        certificateDetails.setCertificateErrors(Collections.singletonList(Collections.singletonMap("thumbprint", new ArrayList<>(
